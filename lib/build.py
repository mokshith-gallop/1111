...
def _remap_datasets(sql: str, dataset_map: dict) -> str:
    """Rewrite qualified cross-dataset refs to the build datasets, so a view's production ref
    `ods.x` resolves to the sandbox `dmt_build_ods.x` during build-and-verify. Matches a mapped
    name only as the leading segment of a qualified ref (optional backticks, word-boundary, a
    following `.`) so it never touches a column, alias, or substring."""
    for real, built in dataset_map.items():
        sql = re.sub(rf"(?<![\w.])`?{re.escape(real)}`?(?=\s*\.)", built, sql)
    return sql


def apply_step(bq, build_ds: str, step: dict, base_dir: str = ".", dataset_map: dict | None = None) -> None:
    """Apply one migration step VERBATIM. Any error propagates (the orchestrator aborts the
    remaining steps — a half-built target must not be judged as complete). A step may name its
    own `dataset` (multi-dataset builds); otherwise it lands in the default build dataset.
    `dataset_map` remaps qualified cross-dataset refs in the SQL (e.g. `ods.x` -> `dmt_build_ods.x`)
    so a view's production refs resolve against the sandbox build datasets."""
    ds = step.get("dataset") or build_ds
    kind = step.get("kind")
    if kind in ("ddl", "transform"):
        sql = _step_sql(step, base_dir)
        if dataset_map:
            sql = _remap_datasets(sql, dataset_map)
        run_sql(bq, ds, sql)
    elif kind == "load":
        _load(bq, ds, step)
    elif kind == "external":
